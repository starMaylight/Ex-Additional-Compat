...
public class HeatContentWidget extends ContentWidget<Double> {

    private static final int COLOR = 0xFFFF6600;

    @Override
    protected void onContentUpdate() {
        if (content != null) {
            setHoverTooltips(String.format("%.1f Heat", content));
        }
    }
    ...
    protected void drawHookBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        font.drawShadow(poseStack, "Ht", (pos.x + 1) * 2f, (pos.y + 1) * 2f, COLOR);
        if (content != null) {
    ...
    public void openConfigurator(WidgetGroup group) {
        ...
        group.addWidget(new LabelWidget(5, 50, "Heat Amount"));
