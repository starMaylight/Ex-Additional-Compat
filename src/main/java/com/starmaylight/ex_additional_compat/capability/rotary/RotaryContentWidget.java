...
public class RotaryContentWidget extends ContentWidget<Double> {

    private static final int COLOR = 0xFF8B8B00;

    @Override
    protected void onContentUpdate() {
        if (content != null) {
            setHoverTooltips(String.format("%.1f J (Rotary)", content));
        }
    }
    ...
    protected void drawHookBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        font.drawShadow(poseStack, "Rt", (pos.x + 1) * 2f, (pos.y + 1) * 2f, COLOR);
        if (content != null) {
    ...
    public void openConfigurator(WidgetGroup group) {
        ...
        group.addWidget(new LabelWidget(5, 50, "Rotary (J)"));
