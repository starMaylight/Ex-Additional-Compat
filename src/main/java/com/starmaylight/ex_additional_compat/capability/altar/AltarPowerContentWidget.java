...
public class AltarPowerContentWidget extends ContentWidget<Double> {

    private static final int COLOR = 0xFF7B2D8B;

    @Override
    protected void onContentUpdate() {
        if (content != null) {
            setHoverTooltips(String.format("%.0f Altar Power", content));
        }
    }
    ...
    protected void drawHookBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        font.drawShadow(poseStack, "AP", (pos.x + 1) * 2f, (pos.y + 1) * 2f, COLOR);
        if (content != null) {
    ...
    public void openConfigurator(WidgetGroup group) {
        ...
        group.addWidget(new LabelWidget(5, 50, "Altar Power"));
