...
public class ChaosContentWidget extends ContentWidget<Integer> {

    private static final int COLOR = 0xFF6A0DAD;

    @Override
    protected void onContentUpdate() {
        if (content != null) {
            setHoverTooltips(String.format("%d Chaos", content));
        }
    }
    ...
    protected void drawHookBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        font.drawShadow(poseStack, "Ch", (pos.x + 1) * 2f, (pos.y + 1) * 2f, COLOR);
        if (content != null) {
    ...
    public void openConfigurator(WidgetGroup group) {
        ...
        group.addWidget(new LabelWidget(5, 50, "Chaos Amount"));
